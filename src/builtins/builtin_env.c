...
/*   Updated: 2025/11/20 18:32:31 by liferrei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    size_t total = ft_strlen(key) + ft_strlen(value) + 2;
    char *var = garbage_calloc(data, total);
    if (!var)
        return (NULL);
...
int env_set(t_shell *data, char ***env, const char *key, const char *value)
{
    ...
    char    *new_var = env_build_var(data, key, value);
    char    **new_env;
...
int env_remove(t_shell *data, char ***env, const char *key)
{
	...
	char **new_env = garbage_calloc(data, sizeof(char *) * count);
	if (!new_env)
		return (1);
	while ((*env)[i])
	{
